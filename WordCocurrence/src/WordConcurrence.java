...
public class WordConcurrence {
	...
	public static class FirstPartitioner extends
		...
		@Override
		public int getPartition(WordPair key, IntWritable value,
				int numPartitions) {
			...
			while (matcher.find()) {
				...
				if (windowQueue.size() >= windowSize) {
					...
					while (it.hasNext()) {
						...
						context.write(new WordPair(w1, "*"), one);
						context.write(new WordPair(next, w1), one);
						context.write(new WordPair(next, "*"), one);
					}
					windowQueue.remove();
				}
			}
	...
	public static class WordConcurrenceReducer extends
		...
		public void reduce(WordPair wordPair, Iterable<IntWritable> frequence,
			...
			if (wordPair.getWordB().equals("*")) {
				context.write(wordPair, new Text(Integer.toString(sum)));
				total = sum;
			} else {
				context.write(wordPair, new Text(Integer.toString(sum) + "/"
	...
	public static void main(String[] args) throws IOException,
		...
		wordConcurrenceJob.setPartitionerClass(FirstPartitioner.class);
		...
		wordConcurrenceJob.setOutputValueClass(Text.class);
